...
async def judge_answer(status, guess, word):
	...
	await start_round(current_game.current_round)
			...
			if current_game.guesser == message.author:
				if not current_game.hint_time: # 정답 추측
					...
					if current_game.guess != "패스":
						...
						await judge_answer("pass", current_game.guess, current_game.word) 
			else:
				if current_game.hint_time: # 힌트 제시
					...
					if current_game.hint_submission >= len(current_game.members) - 1: # 힌트 검수 시작
						...
						embed.add_field(name=f"힌트 검수가 끝났다면, 제시어 {current_game.word}을(를) DM으로 보내주세요!", value="단어를 삭제하고 싶다면, 똑같은 단어를 입력해주세요! 삭제된 힌트는 되돌릴 수 없으니 주의하시고요!", inline=False)
...
async def on_raw_reaction_add(payload):
	...
	if confirmer.id == payload.user_id:
		if str(payload.emoji) == "⭕":
			await judge_answer("correct", current_game.guess, current_game.word)
		elif str(payload.emoji) == "❌":
			await judge_answer("wrong", current_game.guess, current_game.word)
bot.run(token)